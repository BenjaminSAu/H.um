...
Mem_Obj Memory_new()
{
  ...
  assert(mem != NULL);
  mem->segments = malloc(sizeof(uint32_t *) * 16);
  mem->length = 16;
...
void Memory_free(Mem_Obj *mem)
{
  ...
  uint32_t *segment = (*mem)->segments[0];
  free(segment);

  for (int i = 1; i < length; i++)
  {
          segment = (*mem)->segments[i];
          if (segment != NULL)
          {
                  segment--;
                  free(segment);
...
uint32_t Memory_get(Mem_Obj mem, int seg_id, int offset)
{
  assert(mem != NULL);
  if (seg_id != 0) {
    uint32_t *segment = (mem->segments)[seg_id];
...
void Memory_put(Mem_Obj mem, int seg_id, int offset, uint32_t word)
{
  assert(mem != NULL);
  if (seg_id != 0) {
      ((mem->segments)[seg_id])[offset] = word;
      return;
  }
...
uint32_t Memory_map(Mem_Obj mem, int length)
{
  ...
  {
          if(mem->length <= mem->counter)
          {
                  mem->segments = realloc(mem->segments, sizeof(uint32_t *) * (mem->length * 2));
                  mem->length *= 2;
          }
          seg_id = mem->counter++;
  }
  ...
  uint32_t *new_seg = calloc(length + 1, sizeof(uint32_t));
  assert(new_seg != NULL);
...
{
  uint32_t *load_seg = (mem->segments)[seg_id];
  free(mem->seg_zero);
  mem->seg_zero = NULL;
  (load_seg)--;
